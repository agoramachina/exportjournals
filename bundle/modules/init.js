var P=Object.defineProperty;var T=(i,e,t)=>e in i?P(i,e,{enumerable:!0,configurable:!0,writable:!0,value:t}):i[e]=t;var p=(i,e)=>P(i,"name",{value:e,configurable:!0});var k=(i,e,t)=>(T(i,typeof e!="symbol"?e+"":e,t),t);var{TextEditor:L}=foundry.applications.ux,h=class{static async exportPack(e,t){let o=await e.getDocuments(),r=new JSZip;for(let d of o){let v=await this.convertJournalToMarkdown(d,t),g=this.buildPath(d,e,[]);g.push(d.name);for(let a of v){let s=a.name.replace(/[^a-zA-Z0-9-_]/g,"_");r.folder(g.join("/")).file(`${s}.md`,a.markdown)}}let n=e.metadata.label||e.metadata.name||e.metadata.id,l=await r.generateAsync({type:"blob"}),c=document.createElement("a");c.href=URL.createObjectURL(l),c.download=`${n}.zip`,c.click(),URL.revokeObjectURL(c.href)}static buildPath(e,t,o){return e.folder?(o.unshift(e.folder.name),this.buildPath(e.folder,t,o)):o}static async convertJournalToMarkdown(e,t){let o=foundry.applications.sheets.journal.JournalEntryPageTextSheet._converter,r=[];for(let n of e.pages){let l=foundry.utils.getProperty(n,"text.content");if(!l)return"";t.enrich&&(l=await L.enrichHTML(l)),t.removeLinks&&(l=l.replace(/<a[^>]*>(.*?)<\/a>/g,"$1"));let c=o.makeMarkdown(l);r.push({markdown:c,name:n.name})}return r}};p(h,"ExportAsMarkdown");var{TextEditor:j}=foundry.applications.ux,f=class{static async exportPack(e,t,o){let r=await e.getDocuments(),n={none:[]};for(let a of e.folders)for(let s of r.filter(m=>m.folder===a.id)){let w=(await this.convertJournalToHTML(s,t)).map(x=>`
                        <h3>${x.name}</h3>
                        ${x.html}
                    <div class="page_break"></div>`).join(""),y=`
                    <h2>${s.name}</h2>
                    ${w}
                    <div class="page_break"></div>`;n[a.id]=n[a.id]||[],n[a.id].push(y)}for(let a of r.filter(s=>!s.folder)){let m=(await this.convertJournalToHTML(a,t)).map(y=>`
                        <h3>${y.name}</h3>
                        ${y.html}
